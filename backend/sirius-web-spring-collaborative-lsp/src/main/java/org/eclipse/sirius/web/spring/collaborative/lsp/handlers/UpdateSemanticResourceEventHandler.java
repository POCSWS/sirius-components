...
import org.eclipse.sirius.web.spring.collaborative.lsp.dto.UpdateSemanticResourceSuccessPayload;
...
public class UpdateSemanticResourceEventHandler implements ILspTextEventHandler {
    ...
    public UpdateSemanticResourceEventHandler(ICollaborativeMessageService messageService, IObjectService objectService, MeterRegistry meterRegistry) {
    ...
    @Override
    public void handle(One<IPayload> payloadSink, Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, ILspTextContext lspTextContext, ILspTextInput input) {
        ...
        if (input instanceof UpdateSemanticResourceInput) {
            ...
            payloadSink.tryEmitValue(new UpdateSemanticResourceSuccessPayload(updateSemanticResourceInput.getId(), editingContext.getId().toString()));
            changeDescriptionSink.tryEmitNext(new ChangeDescription(changeKind, editingContext.getId(), input));
        } else {
