...
public class SysMLAdHocSerializerTests {
	...
	@ValueSource(strings = { "models/1-EmptyNamespace.xmi", "models/2-NamespaceWithOnePackage.xmi",
			...
			"models/4-PartDefinitionAndPartUsage.xmi", "models/6-PortDefinitionAndPortUsage.xmi" })
	void fromXmiModelSerializeThenParse(String relativePathToXmiModelFile) {
		...
		// models.
		final List<EObject> originalModelContents = getAllContentsOf(loadedModel);
		final List<EObject> parsedModelContents = getAllContentsOf(parsedModel);
		final Supplier<String> failMessageSupplier = () -> failMessageBase + "\nAnd then parsed back into a model with "
				+ parsedModelContents.size() + " contents:\n" + parsedModelContents + "\nBut the original model had "
				+ originalModelContents.size() + " contents:\n" + originalModelContents
		...
		Assertions.assertThat(parsedModelContents.size()).withFailMessage(failMessageSupplier)
				.isEqualTo(originalModelContents.size());
		IntStream.rangeClosed(0, originalModelContents.size() - 1).forEach(index -> {
			Assertions.assertThat(parsedModelContents.get(index).eClass()).withFailMessage(failMessageSupplier)
					.isEqualTo(originalModelContents.get(index).eClass());
		});
	}
...
//				.isEqualTo(source);
	...
	private static List<EObject> getAllContentsOf(EObject eObject) {
		final List<EObject> allContents = new ArrayList<>();
