...
public class SysmlLimitedSerializer extends SysMLSwitch<String> {
	...
	private String ruleUsageElement(Usage usage) {
		if (usage.eClass() == SysMLPackage.Literals.REFERENCE_USAGE) {
			...
			return serialized;
		} else if (usage.eClass() == SysMLPackage.Literals.PART_USAGE) {
			return rulePartUsage((PartUsage) usage);
		} else {
	...
	private String rulePartUsage(PartUsage partUsage) {
		return Stream.of(ruleOccurrenceUsagePrefix(partUsage), "part", ruleUsage(partUsage)).filter(Objects::nonNull)
				.collect(Collectors.joining(" "));
	}
...
//						}
	...
	private String rulePortUsage(PortUsage portUsage) {
		final String serialized = Stream.of(ruleOccurrenceUsagePrefix(portUsage), "port", ruleUsage(portUsage))
				.filter(Objects::nonNull).collect(Collectors.joining(" "));
		consume(portUsage);
		return serialized;
	}
		...
		return Stream
				.of(ruleDefinitionPrefix(portDefinition), "port def", ruleDefinition(portDefinition),
						portDefinition.getOwnedRelationship().stream()
								...
								.filter(Membership.class::isInstance).map(Membership.class::cast)
								.map(this::ruleConjugatedPortDefinitionMember).filter(Objects::nonNull)
								.collect(Collectors.joining("\n")))
				.filter(Objects::nonNull).collect(Collectors.joining(" "));
	}
	...
	private String ruleDefinitionPrefix(Definition definition) {
		if (definition.isAbstract() && definition.isVariation()) {
			return throwExplicitlyNotSupported(definition,
					"Grammar rule 'DefinitionPrefix' does not expect both assignments 'isAbstract' and 'isVariation' to be true.");
		} else {
			return Stream.of(definition.isAbstract() ? "abstract" : null, definition.isVariation() ? "variation" : null)
					.filter(Objects::nonNull).collect(Collectors.joining(" "));
		}
	}
